...
{#each grads as i, idx}
	<div
		class="color-{i} test-bg"
		style="z-index: {grads.length -
			idx}; opacity: 0.01; background-image: linear-gradient(to bottom, var(--color-{i}-1) , var(--color-{i}-2) ); animation-delay:  {idx *
			4 -
			2}s; animation-duration: {grads.length * 4}s;"
	></div>
{/each}
